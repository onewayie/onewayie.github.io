...
$(".main-contact").submit(function(e) {
    ...
    $.ajax({
        ...
        data: form.serialize(), 
        beforeSend: function() {
            $(".main-contact .submit-button").text('Please wait...');
        },
        success: function(data) {
            UIkit.modal('#start-project').hide();
            UIkit.modal('#success-md').show();
            $(".main-contact").trigger('reset');
        }
    }).done(function(){
        $(".main-contact .submit-button").text('Send Brief');
    });
